const CACHE_NAME='familytree-v65-cache-fixed';
const urlsToCache=['./index.html?fresh=65','./manifest.json'];
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(fetch(event.request,{cache:'no-store'}).then(res=>{
    ...
    return res;
  }).catch(()=>caches.match(event.request).then(res=>res||caches.match('./index.html?fresh=65'))));
});
